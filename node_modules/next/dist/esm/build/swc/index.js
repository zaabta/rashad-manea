...
export function createDefineEnv({ isTurbopack, clientRouterFilters, config, dev, distDir, projectPath, fetchCacheKeyPrefix, hasRewrites, middlewareMatchers, rewrites }) {
    let defineEnv = {
        client: [],
        edge: [],
        nodejs: []
    ...
    for (const variant of Object.keys(defineEnv)){
        defineEnv[variant] = rustifyOptionEnv(getDefineEnv({
            isTurbopack,
            clientRouterFilters,
            config,
            dev,
...
async function loadWasmRawBindings(importPath = '') {
    ...
    if (testWasmDir) {
        ...
        const rawBindings = await import(pathToFileURL(path.join(testWasmDir, 'wasm.js')).toString());
        ...
        for (let pkg of [
            ...
            try {
                ...
                const importedRawBindings = await import(pathToFileURL(pkgPath).toString());
                let rawBindings;
        ...
        nativeBindings = {
            ...
            turbo: {
                ...
                startTurbopackTraceServer (traceFilePath) {
                    Log.warn('Turbopack trace server started. View trace at https://trace.nextjs.org');
